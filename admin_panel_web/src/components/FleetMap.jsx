...
const FleetMap = () => {
    ...
    useEffect(() => {
        ...
        const profileSub = supabase.channel('fleet-profile-updates').on('postgres_changes', { event: '*', schema: 'public', table: 'profiles' }, () => { fetchFleetData(); }).subscribe();
        const ticketSub = supabase.channel('fleet-workload-updates').on('postgres_changes', { event: '*', schema: 'public', table: 'tickets' }, () => { fetchFleetData(); }).subscribe();
        const heartbeat = setInterval(() => { fetchFleetData(); }, 60000);
    ...
    const fetchFleetData = async () => {
        const [profilesRes, configRes, ticketsRes] = await Promise.all([
            ...
            supabase.from('tickets').select('technician_id').gte('scheduled_at', new Date(new Date().setHours(0, 0, 0, 0)).toISOString()).lte('scheduled_at', new Date(new Date().setHours(23, 59, 59, 999)).toISOString()).neq('status', 'cancelado')
        ]);
        ...
        const merged = techData.map(t => {
            ...
            return { technician_id: t.id, profiles: t, latitude: t.current_lat || 36.7212, longitude: t.current_lng || -4.4217, workload, isActive, lastUpdate: lastUpdateDate };
        });
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-4 h-auto min-h-[calc(100vh-140px)]">
            {/* RIGHT SIDEBAR (OPERATIONS) - Order 2 on Desktop (Right), Order 2 on Mobile (Bottom) -> Map First! */}
            <div className="order-2 lg:order-2 w-full lg:w-1/3 xl:w-1/4 flex flex-col gap-4">
                <SolarClock workingHours={workingHours} />
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 lg:flex-1 h-[400px] lg:h-auto flex flex-col overflow-hidden">
                    <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                        <h3 className="font-bold text-slate-700 flex items-center gap-2"><User size={16} /> Fuerza de Campo</h3>
                        <span className="text-xs font-bold bg-green-100 text-green-700 px-2 py-0.5 rounded-full border border-green-200">{activeTechsCount} Activos</span>
                    </div>
                    <div className="flex-1 overflow-y-auto p-2 space-y-2">
                        {techs.map(tech => (
                            <div key={tech.technician_id} onClick={() => handleTechClick(tech)} className={`p-3 rounded-lg border transition-all cursor-pointer flex items-center justify-between group ${selectedTech?.technician_id === tech.technician_id ? 'bg-blue-50 border-blue-200 ring-1 ring-blue-300' : 'bg-white border-slate-100 hover:border-blue-200'}`}>
                                <div className="flex items-center gap-3">
                                    <div className="relative">
                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-sm ${tech.isActive ? 'bg-blue-600' : 'bg-slate-300 grayscale'}`}>
                                            {tech.profiles?.avatar_url ? <img src={tech.profiles.avatar_url} className="w-full h-full rounded-full object-cover" /> : (tech.profiles?.full_name?.[0] || 'T')}
                                        </div>
                                    ...
                                    <div>
                                        <h4 className={`text-sm font-bold ${tech.isActive ? 'text-slate-800' : 'text-slate-500'}`}>{tech.profiles?.full_name}</h4>
                                        <div className="flex items-center gap-2 text-[10px] text-slate-400">
                                            {tech.isActive ? <span className="text-green-600 font-medium flex items-center gap-0.5"><Signal size={10} /> Conectado</span> : <span className="flex items-center gap-0.5"><Clock size={10} /> {tech.lastUpdate ? formatDistanceToNow(tech.lastUpdate, { addSuffix: true, locale: es }) : 'Sin señal'}</span>}
                                        </div>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <div className={`text-xs font-black px-2 py-1 rounded-lg text-center min-w-[30px] ${tech.workload > 4 ? 'bg-red-100 text-red-600' : tech.workload > 0 ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-400'}`}>{tech.workload}</div>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            {/* LEFT MAP (75%) - Order 1 on Mobile (Top) */}
            <div className="order-1 lg:order-1 flex-1 bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative h-[400px] lg:h-auto">
                <APIProvider apiKey={GOOGLE_MAPS_API_KEY}>
                    <Map defaultCenter={defaultCenter} defaultZoom={12} mapId="FLEET_CONTROL_MAP" fullscreenControl={false} streetViewControl={false} mapTypeControl={false} onMapInit={({ map }) => setMapInstance(map)}>
                        {techs.map(loc => (
                            <AdvancedMarker key={loc.technician_id} position={{ lat: loc.latitude, lng: loc.longitude }} onClick={() => handleTechClick(loc)}>
                                <div className="relative group">
                                    <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-900/90 backdrop-blur text-white text-[10px] font-bold px-2 py-1 rounded shadow-lg opacity-0 group-hover:opacity-100 transition whitespace-nowrap z-20 pointer-events-none">{loc.profiles?.full_name}</div>
                                    <div className={`w-8 h-8 rounded-full border-2 border-white shadow-xl flex items-center justify-center transform transition duration-300 hover:scale-125 ${loc.isActive ? 'bg-blue-600' : 'bg-slate-400 grayscale'}`}>
                                        {loc.profiles?.avatar_url ? <img src={loc.profiles.avatar_url} className="w-full h-full rounded-full object-cover" /> : <span className="text-white text-xs font-bold">{loc.profiles?.full_name?.[0]}</span>}
                                    </div>
                                    {loc.isActive && <div className="absolute top-0 left-0 w-8 h-8 bg-blue-500 rounded-full -z-10 animate-ping opacity-50"></div>}
                                </div>
                            </AdvancedMarker>
                        ))}
                    </Map>
                ...
                <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm p-3 rounded-xl shadow-lg border border-white/50 z-10 max-w-[200px]">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="flex flex-col">
                            <span className="text-sm font-bold text-slate-700 flex items-center gap-2"><span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>{activeTechsCount} Online</span>
